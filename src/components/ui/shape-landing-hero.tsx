...
const lineFade: Variants = {
  ...
  visible: (i: number) => ({ opacity: 1, y: 0, transition: { duration: 0.7, delay: 0.15 + i * 0.12, ease: [0.16, 1, 0.3, 1] } }),
};
const upFade: Variants = {
  hidden: { opacity: 0, y: 12 },
  visible: (i: number) => ({ opacity: 1, y: 0, transition: { duration: 0.6, delay: 0.5 + i * 0.1, ease: "easeOut" } }),
};
...
export function HeroGeometric({ title1 = "Finance Made", title2 = "Personal" }: {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden bg-[#080d18] px-4">
      {/* Ambient orbs — hidden on mobile to save GPU */}
      <AmbientOrb className="-top-20 left-1/2 -translate-x-1/2" color="rgba(224,93,56,0.07)" size={700} />
      <AmbientOrb className="hidden sm:block top-1/3 -left-40" color="rgba(59,130,246,0.04)" size={450} />
      <AmbientOrb className="hidden sm:block bottom-10 right-10" color="rgba(224,93,56,0.04)" size={300} />

      <div className="absolute inset-0 overflow-hidden">
        <FloatingShape delay={0.4} width={500} height={100} rotate={9}  gradient="from-orange-500/[0.06]" className="left-[-7%] top-[22%]" />
        <FloatingShape delay={0.6} width={340} height={75}  rotate={-13} gradient="from-blue-500/[0.04]"   className="right-[-4%] top-[60%]" />
        <FloatingShape delay={0.5} width={180} height={50}  rotate={-7}  gradient="from-orange-400/[0.05]" className="left-[10%] bottom-[15%]" />
        <FloatingShape delay={0.7} width={120} height={35}  rotate={20}  gradient="from-amber-400/[0.05]"  className="right-[20%] top-[14%]" />
      </div>

      {/* Subtle grid — desktop only */}
      <div aria-hidden className="absolute inset-0 opacity-[0.015] pointer-events-none hidden md:block"
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto text-center flex flex-col items-center">
        <h1 className="font-extrabold leading-none tracking-tight uppercase mb-6 overflow-hidden"
          ...
          <motion.span custom={1} variants={lineFade} initial={r ? "visible" : "hidden"} animate="visible"
            ...
            style={{ background: "linear-gradient(135deg, #ff9a6c 0%, #e05d38 45%, #d94a20 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", willChange: "transform, opacity" }}>
            {title2}
          </motion.span>
        </h1>
        ...
        <motion.p custom={0} variants={upFade} initial={r ? "visible" : "hidden"} animate="visible"
          ...
          style={{ fontFamily: "var(--font-body)", willChange: "transform, opacity" }}>
        ...
        <motion.div custom={1} variants={upFade} initial={r ? "visible" : "hidden"} animate="visible"
          ...
          <a href="#contact" aria-label="Book a strategy call with Moe"
            ...
            style={{ background: "linear-gradient(135deg, #e05d38, #c94822)", boxShadow: "0 0 32px rgba(224,93,56,0.25), 0 1px 0 rgba(255,255,255,0.1) inset, 0 6px 20px rgba(0,0,0,0.25)", minHeight: "52px" }}>
          ...
          <a href="#services" aria-label="See available finance solutions"
            className="inline-flex items-center justify-center w-full sm:w-auto px-7 py-4 rounded-full border border-white/12 text-white/55 hover:text-white/85 hover:border-white/22 font-medium text-sm tracking-wide transition-colors duration-300"
      ...
      <div aria-hidden className="absolute bottom-8 left-1/2 -translate-x-1/2">
        <motion.div animate={r ? {} : { y: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 2.5 }}>
          <ChevronDown className="w-4 h-4 text-white/15" />
        </motion.div>
      </div>
      <div aria-hidden className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 80% 80% at 50% 50%, transparent 30%, rgba(8,13,24,0.6) 100%)" }} />
    </div>
  );
}
