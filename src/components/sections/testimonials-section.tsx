...
export function TestimonialsSection() {
  return (
    <div id="testimonials" className="py-16 sm:py-24" style={{ backgroundColor: "#F8FAFC" }}>
      <div className="container mx-auto">
        <div className="max-w-xl mx-auto text-center mb-12">
          <span className="label-chip-navy mb-4 inline-block">Client Reviews</span>
          <h2
        ...
        >
          {testimonials.map(({ id, name, profession, rating, quote }) => (
            <li
              key={id}
              className="swipe-card flex flex-col p-6 sm:p-7 rounded-xl transition-all duration-200"
              style={{
            ...
            >
              ...
              >
                {Array.from({ length: rating }).map((_, i) => (
                  <Star key={i} className="w-4 h-4" style={{ fill: "#F4C542", color: "#F4C542" }} aria-hidden />
                ))}
              </div>
              <blockquote className="text-sm sm:text-base leading-relaxed mb-5 flex-1" style={{ color: "#475569" }}>
